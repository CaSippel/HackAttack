...
/* confirm('How to:\n1) Choose a square on your side\n2) Try to unlock the screen\n3) On success, create a new pattern for your opponent\n\nWould you like to Start?' */
    ...
    var lock = new PatternLock('#patternContainer', {
      ...
      onDraw:function(pattern) {
        if(entry){
          ...
          var stufz;
          myDataRef.child("P2puzzle").once("value", function(snapshot) {
            stufz = snapshot.val();
            console.log(stufz);
            if (pattern == stufz) {
              //show alert
    ...
    myDataRef.child("T_P2").on("child_added", function(s) {
      if(!entry){
        myDataRef.child("P2enabled").once("value", function(snapshot) {
